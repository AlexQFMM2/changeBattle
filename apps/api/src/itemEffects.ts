...
export function applyTrainingItemToPokemonV4(input: {
  ...
  if (effect.kind === "ev") {
    ...
    const result = applySpecialMedicineEffectV4({
      pokemon: before,
      ...
      rng: createStableRng(input.rngSeed || `${before.localPokemonId}:${before.speciesId}:${input.item.id}:${input.item.itemID}:${effect.medicineId}`),
    });
...
type SpecialMedicineAbilityV4 = {id: string; name?: string; nameZh?: string; hidden?: boolean};

function applySpecialMedicineEffectV4(input: {
  pokemon: LocalPokemonV4;
  medicineId: string;
  pokemonDetail: DexPokemonDetail | null;
  allAbilities: SpecialMedicineAbilityV4[];
  calculateMaxHp?: (pokemon: LocalPokemonV4) => number;
  rng: () => number;
}): {ok: true; pokemon: LocalPokemonV4; message: string} | {ok: false; reason: string} {
  const medicineId = normalizeItemId(input.medicineId);
  const name = input.pokemon.nameZh || input.pokemon.name;
  switch (medicineId) {
    ...
    case "steeltonic": {
      ...
      pokemon = drainRandomMovePp(pokemon, 1, input.rng);
      return {ok: true, pokemon, message: `${name} 使用钢铁药剂，身体沉重下来，1 个技能的 PP 归 0。`};
    }
    case "boletefluid": {
      let pokemon = drainRandomMovePp(input.pokemon, 3, input.rng);
      pokemon = {
        ...pokemon,
        ivs: fullIvTable(31),
        evs: evSpreadForBattleStyle(pokemon, input.rng),
        entryStatus: "psn",
      };
      pokemon = recalculateMaxHp(pokemon, input.calculateMaxHp);
    ...
    case "strangeherb": {
      ...
      const ability = pickDifferentAbility(input.allAbilities, input.pokemon.abilityId, input.rng);
    ...
    case "pulseplasma": {
      let pokemon = {
        ...input.pokemon,
        nature: speedNatureForPokemon(input.pokemon, input.rng),
...
function transferIvEvPairs(pokemon: LocalPokemonV4, pairs: Array<[DexStatId, DexStatId]>): LocalPokemonV4 {
...
function fullIvTable(value: number): StatTableV4 {
  return Object.fromEntries(STAT_IDS.map(stat => [stat, clampInt(value, 0, 31)])) as StatTableV4;
}

function fullEvTable(value: number): StatTableV4 {
  return Object.fromEntries(STAT_IDS.map(stat => [stat, clampInt(value, 0, 252)])) as StatTableV4;
}
...
function evSpread(order: DexStatId[]): StatTableV4 {
...
function shuffleStats(rng: () => number): DexStatId[] {
...
function normalizeEvTotal(evs: StatTableV4, protectedStats: DexStatId[] = []): StatTableV4 {
  const next = Object.fromEntries(STAT_IDS.map(stat => [stat, clampInt(evs[stat], 0, 252)])) as StatTableV4;
  let overflow = statTotal(next) - 510;
  const protectedSet = new Set(protectedStats);
  for (const stat of STAT_IDS.filter(stat => !protectedSet.has(stat)).sort((a, b) => next[b] - next[a])) {
    if (overflow <= 0) break;
...
function reduceTotalEv(evs: StatTableV4, amount: number): StatTableV4 {
  ...
  for (const stat of [...STAT_IDS].sort((a, b) => next[b] - next[a] || STAT_IDS.indexOf(a) - STAT_IDS.indexOf(b))) {
    if (remaining <= 0) break;
...
function addEvToRandomStat(pokemon: LocalPokemonV4, amount: number, rng: () => number): LocalPokemonV4 {
  const candidates = STAT_IDS.filter(stat => pokemon.evs[stat] < 252);
...
function pickDifferentAbility<T extends SpecialMedicineAbilityV4>(abilities: T[], currentId: string, rng: () => number): T | null {
  const normalizedCurrent = normalizeItemId(currentId);
  const pool = abilities.filter(ability => ability.id && normalizeItemId(ability.id) !== normalizedCurrent);
  return pickOne(pool.length ? pool : abilities, rng) || null;
}

function applyAbility(pokemon: LocalPokemonV4, ability: SpecialMedicineAbilityV4): LocalPokemonV4 {
  return {
    ...pokemon,
...
function moveCategoryCount(pokemon: LocalPokemonV4, category: "physical" | "special"): number {
  return pokemon.moves.filter(move => {
    const normalized = normalizeItemId(move.category);
...
function statTotal(stats: StatTableV4): number {
  return STAT_IDS.reduce((sum, stat) => sum + clampInt(stats[stat], 0, 252), 0);
}
