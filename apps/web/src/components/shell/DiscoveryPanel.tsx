...
import type {MainMenuDexCard, MainMenuShopItem} from "./mainMenuTypes";
import {mainMenuCardVariants, mainMenuHomePanelVariants} from "./FavoritePokemonPanel";
import "./DiscoveryPanel.css";

function ItemIcon({item}: {item: MainMenuShopItem}) {
  const style = item.iconStyle ? styleFromCss(item.iconStyle) : undefined;
  if (item.iconStyle) return <span className="item-icon" aria-hidden="true" style={style} />;
  return item.iconUrl ? <img className="item-icon" src={item.iconUrl} alt={item.name_zh || item.name} /> : <span className="item-icon">?</span>;
}
