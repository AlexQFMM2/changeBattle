...
const SPIN_TRANSITION: Transition = {ease: "linear", duration: 0.33, rotate: {inherit: true, ease: [0.57, 0.44, 0.66, 1.17]}};
const EXPAND_TRANSITION: Transition = {type: "spring", duration: 0.55, bounce: 0.05, borderRadius: {inherit: true, ease: "linear"}, scale: {inherit: true, ease: "backOut"}, rotate: {type: false}};

const shellVariants: Record<AnimationPhase, TargetAndTransition> = {
  idle: {scale: 0.42, borderRadius: 75, opacity: 0},
  entry: {...baseValues, borderRadius: 75, transition: {opacity: {duration: 0.08}, duration: 0.13}},
  spin: {...baseValues, borderRadius: 50, rotate: [-180, 0], transition: SPIN_TRANSITION},
  expand: {...baseValues, borderRadius: 24, transition: EXPAND_TRANSITION},
...
const contentVariants: Record<AnimationPhase, TargetAndTransition> = {
  ...
  entry: {opacity: 0, y: 4},
  spin: {opacity: 0, y: 4},
  expand: {opacity: 1, y: 0, transition: {opacity: {duration: 0.12, ease: "linear"}, delayChildren: 0.08, staggerChildren: 0.055}},
  exit: {opacity: 0, y: 3},
};
