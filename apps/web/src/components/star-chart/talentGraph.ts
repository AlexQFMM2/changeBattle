...
export function talentNodeLevel(nodeById: Map<string, StarChartNodeViewV4>, id: string): number {
  return Math.max(0, Math.floor(Number(nodeById.get(id)?.level || 0)));
}

export function talentNodeReady(nodeById: Map<string, StarChartNodeViewV4>, node: StarChartNodeViewV4): boolean {
  return (node.requires || []).every(requirement => talentNodeLevel(nodeById, requirement.id) >= Math.max(1, Number(requirement.level || 1)));
}

export function talentNodeCost(node?: StarChartNodeViewV4): number | null {
...
export function talentNodeState(nodeById: Map<string, StarChartNodeViewV4>, node: StarChartNodeViewV4): TalentNodeState {
...
export function talentGraphBounds(catalog: StarChartNodeViewV4[]): TalentGraphBounds {
...
export function talentCategoryHubs(catalog: StarChartNodeViewV4[]): Map<string, TalentCategoryHub> {
  ...
  const groups = new Map<string, StarChartNodeViewV4[]>();
...
export function talentRequirementText(nodeById: Map<string, StarChartNodeViewV4>, requirement: {id: string; level?: number}) {
