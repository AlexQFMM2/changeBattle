...
export function FormalGameTransitionPage({api, profile, mode, onRunReady}: {
  ...
  useEffect(() => {
    ...
    void api.saveFormalGameRun(prepared).then(onRunReady);
  }, [api, mode, onRunReady, partnerPreference, profile]);

  return (
    <TrainingRunTransitionPage
      title="准备正式游戏"
      detail={modeLabel(mode)}
      tip={mode === "coop" ? "合作模式本轮先记录 AI 队友偏好，后续进入 7 场计划时生成精英队友。" : "正在生成开局候选宝可梦。"}
      onReady={() => undefined}
    />
  );
}
