...
- [x] 压上杠杆（5000BP）：每场胜利有概率把本场基础 BP 变为 1.4 / 1.6 / 1.8 / 2.0 / 2.5 倍；若本局失败，则不获得背包结算返还，并将 BP 回到本局开始前状态。
...
- [x] 开局 6 选 3 按历史连续通关数生成：连胜 0 为 `4 个一阶 + 2 个二阶`；连胜 1 为 `3 个一阶 + 2 个二阶 + 1 个三阶`；连胜 >=2 为 `1 个一阶 + 2 个二阶 + 3 个三阶`。
