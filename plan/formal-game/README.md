...
| 文档 | 状态 | 说明 |
| --- | --- | --- |
| [formal-soulmate-egg-hatch-and-growth-plan.md](formal-soulmate-egg-hatch-and-growth-plan.md) | 主体已落地，后续回归 | “就决定是你了”后的蛋生成规则已抽到 core helper，并能写入长期宝可梦箱；仓库进化、全局进化弹窗、真实对局同行许可和战后亲密度回写已接入，后续主要看休整页提示和回归。 |
...
- 回归“灵魂伴侣”MVP：最终胜利带走蛋、仓库养成/技能学习/进化、同行许可带入正式流程、正式休整页禁用态和战后亲密度回写。
- 隐藏登场、徽章、战斗内进化和更多亲密度来源归入后续扩展，不进入当前收口范围。
