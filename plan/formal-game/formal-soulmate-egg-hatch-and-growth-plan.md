...
6. Web 新增蛋孵化动画组件，并接入 `assets/runtime/soulmate/egg-hatch-sheet.png` 序列图。
7. Web 成功后提示已送入玩家宝可梦箱子。
8. 训练家仓库承接灵魂伴侣局外养成入口。
9. 仓库进化道具预览、确认、全局进化弹窗和 vault 写回已接入。
10. 最后再考虑隐藏战斗进化。

## Assumptions

- 本轮先接“带走并生成局外宝可梦”闭环，隐藏战斗进化不做。
- 孵化动画当前使用 `assets/runtime/soulmate/egg-hatch-sheet.png` 序列图；后续可以继续替换更精细的演出素材，但流程不再是纯占位。
...
- 战后亲密度成长已进入真实对局接入：通过同行许可带入正式流程的 run-local 副本仍不参与局内养成，但胜利、参战和阵亡等结果会按 node marker 幂等回写到长期资产亲密度。
