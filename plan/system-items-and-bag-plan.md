...
截至 2026-06-28，本计划覆盖的“系统战斗道具 + 统一背包 + 休整页道具使用 + Battle V4 恢复道具”主体功能已经完成。后续新增内容不再继续堆到本计划中；奖励来源、商店经济、道具生命周期、AI 使用道具等外层能力应拆新计划追踪。

## Current Batch Status

截至 2026-06-28，背包已从“实例展示/占位”推进到“统一组件 + 休整页常用道具可用 + Battle V4 恢复道具可用 + 系统战斗道具重铸可用”的阶段。休整页背包 V1 交互已完成，后续重点转为奖励/商店/道具生命周期等外层系统。
...
- 休整页背包动作按钮按当前道具能力显示；不可用动作不再占位，例如恢复药不显示“立即携带”。
...
| 休整页背包 | 统一面板、完整背包、按道具能力显示动作按钮、携带/更替/卸下、丢弃、恢复类使用、训练道具使用、TM 替换技能、系统道具重铸、成功 toast | 携带道具过期/报废 |
