...
玩家/NPC 立绘进场、台词、胜负后再进场台词、离开页面等“非竞技式演出流程”已进入下一阶段。下一步先实现“训练家立绘进场 -> 台词 -> 进入战斗”，正式入口和训练场入口都显示，按 V1 的点击/空格/Enter 推进。
...
- 下一阶段演出目标：训练家立绘进场 -> 台词 -> 进入战斗。覆盖正式流程和训练场调试入口，复刻 V1 点击画面/空格/Enter 推进台词。
- 未来完整演出目标：胜利/失败后训练家立绘重新进场 -> 台词 -> 离开战斗页进入中转页。
...
- 投降框和投降流程参数已完成；下一步实现训练家立绘/台词入口流程。
