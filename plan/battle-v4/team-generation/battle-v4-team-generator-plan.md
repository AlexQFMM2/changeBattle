...
- 闪光概率固定为 `1/30`。
- 候选数量由正式 run 创建时冻结的 `starChartSnapshot` 决定：root-only 为 6 只，点亮“多多益善 I-IV”后逐步提升到 7/8/9/10 只。
- 基础 6 只角色固定为：天气组件、空间组件、攻击手、攻击手、辅助手、防御手。
- 星图随机扩展位不重复天气/空间保底，第 7-10 位为：速度控制/节奏手、干扰/场地/撒场手、第二防御或辅助补位、泛用攻击或队伍短板补位。
