...
ChangeBattle V2 是当前主线。`v2` 分支对应 beta/debug 通道，当前版本是 `0.1.28`。
...
- `docs/release-notes-v0.1.28.md`：当前 beta 版本说明。
...
- Windows Desktop portable release：debug 桌面端主发布链路已迁到 GitHub Actions；完整 zip 由 GitHub Release 托管，更新 metadata artifact 下载到本地后发布到自有服务器。`0.1.28` 是当前正式房间 scoped view 同步遮罩和 Desk 资源协议诊断修复 beta 基线；线上服务器只放增量 metadata 和下载页，Android debug APK 当前仍为 0.1.26。
...
latest debug: 0.1.28
GitHub Release: https://github.com/AlexQFMM2/changeBattle/releases/tag/desk-debug-v0.1.28
Desktop package: ChangeBattle-V2-Desk-portable-debug-v0.1.28.zip
...
`0.1.28` 是当前正式房间 scoped view 同步遮罩和 Desk 资源协议诊断修复 beta 基线。完整包托管 GitHub Release；beta 服务器只保留 `latest.json`、下载页、manifest 和增量 objects，不托管大 zip/apk。
