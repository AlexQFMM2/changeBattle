...
ssh "$UPDATE_HOST" "sudo mkdir -p '$UPDATE_WEB_ROOT' && sudo install -m 0644 '$REMOTE_TMP_DIR/latest.json' '$UPDATE_WEB_ROOT/latest.json' && sudo install -m 0644 '$REMOTE_TMP_DIR/index.html' '$UPDATE_WEB_ROOT/index.html' && if [ -d '$REMOTE_TMP_DIR/image' ]; then sudo rm -rf '$UPDATE_WEB_ROOT/image' && sudo mkdir -p '$UPDATE_WEB_ROOT/image' && sudo cp -a '$REMOTE_TMP_DIR/image/.' '$UPDATE_WEB_ROOT/image/'; fi"
