...
const charizardEvolutionTree = dex.getPokemonEvolutionTree("charizard");
assert.equal(charizardEvolutionTree.root?.id, "charmander");
assert.deepEqual(charizardEvolutionTree.chain.map(entry => entry.id), ["charmander", "charmeleon", "charizard"]);
assert.ok(charizardEvolutionTree.edges.some(edge => edge.fromSpeciesId === "charmander" && edge.toSpeciesId === "charmeleon" && edge.evoLevel === 16));
assert.ok(charizardEvolutionTree.edges.some(edge => edge.fromSpeciesId === "charmeleon" && edge.toSpeciesId === "charizard" && edge.evoLevel === 36));
const eeveeEvolutionEdges = dex.getPokemonEvolutionEdges("eevee");
assert.ok(eeveeEvolutionEdges.length > 3);
assert.ok(eeveeEvolutionEdges.some(edge => edge.toSpeciesId === "flareon" && edge.evoType === "useItem" && edge.evoItem === "Fire Stone" && edge.evoItemId === "firestone"));
assert.ok(dex.getPokemonEvolutionEdges("kadabra").some(edge => edge.toSpeciesId === "alakazam" && edge.evoType === "trade"));
...
const fireStone = dex.getItemDetail("firestone");
assert.equal(fireStone.kind, "evolution");
assert.equal(fireStone.kindLabel, "进化道具");
assert.equal(fireStone.nameZh, "火之石");
assert.equal(fireStone.canUseToPokemon, true);
const universalEvolutionStone = dex.getItemDetail("universal-evolution-stone");
assert.equal(universalEvolutionStone.kind, "evolution");
assert.equal(universalEvolutionStone.nameZh, "通用进化石");
const linkingCord = dex.getItemDetail("linking-cord");
assert.equal(linkingCord.kind, "evolution");
assert.equal(linkingCord.nameZh, "通讯绳");
