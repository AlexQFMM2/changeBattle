...
3. 显示 A 的 HP 条降到 0。
4. 提示：A 倒下了。
5. 播放 A 的濒死动画，提交 `fainted` 展示状态。
6. 继续播放本回合剩余结算，例如天气、场地、道具、特性和其他文字。
7. 回合结算全部结束后，读取最终 request。
8. 如果 request 是 `forceSwitch`，再弹出不可关闭的换人界面。

强制换人弹窗不能在第 3 步或第 4 步提前出现。它属于“回合结算结束后的 UI 决策”，必须等 timeline queue 播完。

### 场景 11：极巨化结束后恢复原图

期望展示：

1. 极巨 / 超极巨状态下继续播放本回合行动和结算。
2. 如果 Showdown 在本回合末返回 `|-end|...|Dynamax`，先播放本回合此前的所有伤害、回复、状态、天气、场地、道具和特性事件。
3. 提示：A 的极巨化结束了。
4. 播放极巨化结束动画。
5. 动画完成后，才把 A 的展示从 G-Max / 放大状态恢复为原物种原立绘。
6. 再根据最终 request 决定开放操作、自动推进或弹出强制换人。

前端不能直接用“回合结算后的最终 tracker”覆盖正在播放的 active 展示；否则会在回合刚开始播放时就提前变小。播放期间应维护独立的 active 展示快照，并只在 `form` / `switch` / `faint` 等 state step 播到时提交。
