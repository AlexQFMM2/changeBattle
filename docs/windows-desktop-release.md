...
`0.1.25` 是当前 beta hotfix 基线：Desktop zip / Android debug APK 挂 GitHub Release，线上 beta 服务器只发布 `latest.json / index.html / manifests / objects`。本轮包含正式 room 休整规则、战斗提交失败态、投降失败不假结算和 NPC 对手立绘修复。
